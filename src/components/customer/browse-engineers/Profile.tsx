...
import InterviewSlotsModal from '@/components/customer/browse-engineers/modals/Interviewslotsmodal';
...
export default function EngineerProfile() {
  ...
  // Handle interview slots modal
  const handleViewInterviewSlots = () => {
    if (profile?.customerInterview) {
      // Don't allow clicking if customer interview is already completed
      return;
    }
  ...
  useEffect(() => {
    const handleEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        if (isVideoModalOpen) {
          closeVideoModal();
        } else if (showInterviewSlots) {
          setShowInterviewSlots(false);
        }
      }
    };

    if (isVideoModalOpen || showInterviewSlots) {
  ...
  }, [isVideoModalOpen, showInterviewSlots]);
  ...
  return (
    <>
      <div className="min-h-[calc(100vh-14.5625rem)] bg-white w-full pt-8 pb-16 px-4">
        <div className="w-full container mx-auto rounded-lg">
          {/* Back Button */}
          <button
            onClick={handleBack}
            className="flex items-center gap-2 mb-6 px-4 py-2 text-[#2563EB] hover:bg-blue-50 rounded-lg transition-colors group"
          >
            <ArrowLeft
              size={20}
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="w-full lg:w-1/4 flex flex-col items-center">
              ...
              <div className="mt-6 space-y-3 w-full xl:w-4/5 mx-auto">
                ...
                {/* View Interview Slots Button */}
                <button
                  onClick={
                    profile.customerInterview
                      ? undefined
                      : handleViewInterviewSlots
                  }
                  ...
                  disabled={profile.customerInterview}
                >
                  <Calendar
                    size={18}
                    className={profile.customerInterview ? 'text-white' : ''}
                  />
                  <span>
                    {profile.customerInterview
                      ? 'Interview Completed'
                      ...
                      {profile.skills && profile.skills.length > 0 ? (
                        <div className="flex flex-wrap gap-3">
                          {profile.skills.map((skill, index) => (
                            <div
                              key={index}
                              className="px-4 py-2 bg-gray-100 text-gray-800 rounded-lg border border-gray-200 hover:bg-gray-200 transition-colors"
                            >
      ...
      {/* Interview Slots Modal - Use profile.user.id as candidateId */}
      <InterviewSlotsModal
        isOpen={showInterviewSlots}
        onClose={() => setShowInterviewSlots(false)}
        interviewPreferences={profile?.interviewPreferences}
        candidateName={candidateName}
        candidateId={profile?.user?.id || ''} // Use profile.user.id
      />
    </>
  );
}
